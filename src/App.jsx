    ...
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#fdfdfd' }}>
      ...
      <header style={{
        ...
        <div className="container" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '10px', cursor: 'pointer' }} onClick={() => { setCurrentView('home'); setIsBookingOpen(false); setIsTrackingOpen(false); navigate('/'); }} className="nav-link">
          ...
          <nav className="desktop-nav">
            <span className={`nav-link ${currentView === 'home' && location.pathname === '/' ? 'active' : ''}`} onClick={() => { setCurrentView('home'); setIsBookingOpen(false); setIsTrackingOpen(false); navigate('/'); }}>Home</span>
            <span className="nav-link" onClick={() => { setIsBookingOpen(true); navigate('/'); }}>Services</span>
            <span className="nav-link" onClick={() => { setIsBookingOpen(true); navigate('/'); }}>How It Works</span>
            <span className="nav-link" onClick={() => { setIsTrackingOpen(true); setIsBookingOpen(false); navigate('/'); }}>Track Repair</span>
            <span className={`nav-link ${currentView === 'about' && location.pathname === '/' ? 'active' : ''}`} onClick={() => { setCurrentView('about'); setIsBookingOpen(false); setIsTrackingOpen(false); navigate('/'); }}>About Us</span>
            <span className={`nav-link ${currentView === 'contact' && location.pathname === '/' ? 'active' : ''}`} onClick={() => { setCurrentView('contact'); setIsBookingOpen(false); setIsTrackingOpen(false); navigate('/'); }}>Contact</span>
          </nav>
              ...
              <div style={{ position: 'relative' }}>
                ...
                {isLoginDropdownOpen && (
                  <div style={{
                    ...
                    minWidth: '160px', border: '1px solid #eee'
                  }}>
                    <span className="dropdown-item" style={{ padding: '10px 16px', cursor: 'pointer', borderBottom: '1px solid #eee', fontSize: '0.9rem', color: '#000' }} onClick={() => { setIsLoginDropdownOpen(false); navigate('/customer-login'); }}>Customer</span>
                    <span className="dropdown-item" style={{ padding: '10px 16px', cursor: 'pointer', borderBottom: '1px solid #eee', fontSize: '0.9rem', color: '#000' }} onClick={() => { setIsLoginDropdownOpen(false); navigate('/technician'); }}>Technician</span>
                    <span className="dropdown-item" style={{ padding: '10px 16px', cursor: 'pointer', fontSize: '0.9rem', color: '#000' }} onClick={() => { setIsLoginDropdownOpen(false); navigate('/admin'); }}>Admin</span>
                  </div>
                )}
              </div>
            )}
            <button className="btn btn-black" onClick={() => { setIsBookingOpen(true); navigate('/'); }} style={{ padding: '8px 20px', display: 'flex', alignItems: 'center', gap: '8px' }}>
              Book a Service
              <div style={{
      ...
      <Routes>
        ...
        <Route path="/customer-login" element={
          <div className="container" style={{ padding: '40px 20px' }}>
            {!currentUser ? (
              <Auth targetRole="customer" onLogin={(user) => { setCurrentUser(user); navigate('/'); }} showToast={showToast} />
            ) : (
              <div style={{ textAlign: 'center', padding: '100px 20px', background: '#fff', borderRadius: '16px', boxShadow: '0 4px 12px rgba(0,0,0,0.05)' }}>
                <CheckCircle size={48} color="var(--success)" style={{ margin: '0 auto 20px' }} />
                <h2 style={{ fontSize: '1.5rem', fontWeight: 'bold', marginBottom: '10px' }}>Already Logged In</h2>
                <p style={{ color: 'var(--text-secondary)' }}>You are signed in as {currentUser.full_name}.</p>
                <button className="btn btn-primary" style={{ marginTop: '20px', padding: '12px 24px' }} onClick={() => navigate('/')}>Return Home</button>
              </div>
            )}
          </div>
        } />
